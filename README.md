...
- TOML 配置结构
- 默认配置预设已内置进 `app.exe`，即使脱离仓库目录也能直接加载 `configs/master.toml` / `configs/slave.toml` / `configs/peer.toml`
- 实时处理主循环骨架
...
- Windows GUI 壳，支持查看设备列表、把 `input_device` / 主输出设备 / 第二输出设备 / 模式与路由保存回配置文件，并在运行中请求 runtime reload / 自动重试恢复
...
- GUI 现在会把思源黑体 CN 的 regular / bold 直接编进 `app.exe`；默认界面统一使用思源黑体，`Start` 按钮单独使用 bold
...
- `Realtime Metrics` 现在支持四档尺寸：`极简`、`默认`、`大`、`我瞎了`；默认档位会把 4 个指标面板排成一行，`极简` 档则只保留文字摘要，不显示图表和进度条
...
- `configs/master.toml`: 主从直播场景的主机预设
- `configs/slave.toml`: 主从直播场景的从机预设
- `configs/peer.toml`: 对等场景预设；如需 `both/both`，把 `session_mode` 改成 `both`
...
cargo run -p offline_replay -- configs/peer.toml 180
cargo run -p runtime_smoke -- configs/master.toml 20
cargo run -p runtime_smoke -- configs/peer.toml 20
...
`input_device` / `primary_target_device` / `secondary_target_device`。

如果更习惯在 GUI 里操作，也可以直接启动 GUI，使用左侧的设备列表点选目标设备。
左侧的 `配置文件` 现在是直接下拉选择；选中预设后会立刻重新装入配置，不再需要额外点击 `Load Config` 或 `刷新配置`。`Import Config Folder` 仍可打开 Windows 文件夹选择界面批量导入整个 config 文件夹。
GUI 对 `configs/master.toml` 这类相对路径会优先按仓库根目录解析；如果脱离仓库单独携带 `app.exe`，则会回退到 exe 所在目录，并直接使用内置配置预设。
切换 `配置文件` 下拉后，会把当前 TOML 的设备字段和网络字段重新装入界面，并在 runtime 运行中自动请求 reload。
...
- `master/slave`: 单人双麦直播；主机可选混音到主输出或本地/对端分离到两个输出设备，从机可选本地输出或关闭输出
- `peer/peer`: 双人互不影响；每台机器只输出自己的处理后声音
- `both/both`: 双人双向监听；每台机器都可选择混音到主输出或本地/对端分离到两个输出设备

严格限制：

- `master` 只能和 `slave` 配对
- `peer` + `session_mode = "peer"` 只能和 `peer` + `session_mode = "peer"` 配对
- `peer` + `session_mode = "both"` 只能和 `peer` + `session_mode = "both"` 配对
